...
haar_faces = cv2.CascadeClassifier(A_參數設定.HAAR_FACES)
...
if __name__ == '__main__':
    ...
    if not os.path.exists(A_參數設定.POSITIVE_DIR):
        # 新增 config.POSITIVE_DIR 指定的目錄
        os.makedirs(A_參數設定.POSITIVE_DIR)

    # 找出 config.POSITIVE_DIR 指定的目錄符合檔案名規則的檔案
    files = sorted(glob.glob(os.path.join(A_參數設定.POSITIVE_DIR,
                                          A_參數設定.POSITIVE_FILE_PREFIX + '[0-9][0-9][0-9].pgm')))
    # 設定計算器為 0
    count = 0
    ...
    while True:
        ...
        # 進行判斷是否是單一人臉的相片
        result = A_參數設定.detect_single(gray, haar_faces)
        ...
        # 進行裁切
        crop = A_參數設定.crop(gray, x, y, w, h)

        # 產生新編檔名
        filename = os.path.join(A_參數設定.POSITIVE_DIR, A_參數設定.POSITIVE_FILE_PREFIX + '%03d.pgm' % count)
